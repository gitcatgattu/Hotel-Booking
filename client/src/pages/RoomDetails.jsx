...
const checkAvailability = async () => {
  try {
    ...
    // ✅ Get Clerk token (if using Clerk)
    const token = await getToken();

    const { data } = await axios.post(
      "/api/bookings/check-availability",
      {
        roomId: id, // ✅ match backend key
        checkInDate,
        checkOutDate,
      },
      {
...
rounded-1g bg-gray-100"
        ...
        <button className="px-3 py-1.5 mt-4 rounded text-white bg-primary hover:bg-primary-dull transition-all cursor-pointer">
          Contact Now
        </button>
      </div>
    )
